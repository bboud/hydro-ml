...
def plot_telemetry(disc_loss_total, gen_loss_total, real_total, fake_total, data, generated_data):
    ...
    ax2.plot(data[0][0], label="Real Curve", color='blue')
    ax2.plot(generated_data[0][0], label="Generated Curve", color='red')
    ax2.set_title("Generator vs Actual Output")
    ax2.set_ylabel("$dN^{ch}/d\eta$")
    ax2.set_xlabel("$\eta$")
    ax2.set_ylim([-0.5, 30.05])
    ax2.legend()

    ax3 = fig.add_subplot(gs[2,1])
    ax3.plot(data[1][0], label="Mean Real Output", color='blue')
    ax3.plot(generated_data[1][0], label="Mean Fake Output", color='red')
    ax3.set_title("Generator vs Actual Output")
    ax3.set_ylabel("$dN^{ch}/d\eta$")
    ax3.set_xlabel("$\eta$")
    ax3.set_ylim([-0.5, 30.05])
    ax3.legend()
